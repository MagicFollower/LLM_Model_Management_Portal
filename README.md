...
![文档上传与切分预览](images/6.png)

文档上传后自动切分，右侧预览区展示片段内容与参数配置，支持调整切分长度与重叠长度后重新处理。
...
本地向量检索测试面板，支持 Top-K、最低分数配置，BM25 关键词检索与 Reranker 重排序开关，结果按归一化分数 [0, 100] 排序展示。
...
- BM25 检索：`[0, +∞)`（Okapi 评分）
- RRF 融合：`(0, 0.02]`（排名加权）
- Reranker：`[0, 1]`（sigmoid 归一化）

前端显示 4 位小数（如 `0.7523`），不够直观；最低分数输入步长为 1，无法精确配置。
...
<!-- 最低分数输入步长 -->
return { min: 0, max: 100, step: 0.01, default: 30, hint: '...' }
```
...
| BM25 | Reranker | 最低分数状态 | 原因 |
|:---:|:---:|:---:|---|
| 关 | 关 | ✅ 可配置 | 向量相似度归一化 [0, 100] |
| 开 | 关 | ❌ 禁用 | RRF 排名加权，按 topK 截断，无需阈值 |
| 关 | 开 | ✅ 可配置 | Reranker 归一化 [0, 100] |
| 开 | 开 | ✅ 可配置 | Reranker 归一化 [0, 100]（最终分数来源） |
...
| 最终阶段 | 分数范围 | minScore 配置 | 显示格式 |
|---|---|---|---|
| 纯向量检索 | `[0, 100]`（归一化） | 30 | `85.72 分` |
| BM25 开启（RRF 融合） | `[0, 100]`（归一化） | 0（RRF 已是排序） | `92.15 分` |
| Reranker 开启（任意场景） | `[0, 100]`（归一化） | 30 | `78.43 分` |
...
所有分数最终统一归一化到 `[0, 100]` 区间。

| 阶段 | 原始分数范围 | 归一化方式 | 适用 minScore |
|---|---|---|---|
| 向量检索 | `[-1, 1]`（余弦相似度） | 截断负值 → [0,1] → ×100 | 30（默认） |
| BM25 检索 | `[0, +∞)`（Okapi 评分） | 按批次最大值归一化 → ×100 | 不适用（内部过滤 >0） |
| RRF 融合 | `(0, 0.02]`（排名加权） | 按批次最大值归一化 → ×100 | 0（跳过阈值） |
| Reranker | `[0, 1]`（sigmoid 归一化） | ×100 | 30（可调整） |

> **边界包含**：minScore 过滤使用 `score >= minScore - ε`（ε=1e-9），确保恰好等于阈值的分数不被浮点精度误排除。

##### 模式组合

| BM25 | Reranker | 实际流程 | 最终分数来源 |
|:---:|:---:|---|---|
| 关 | 关 | 向量 → minScore → topK | 向量相似度归一化 [0, 100] |
| 开 | 关 | 向量 + BM25 → RRF → topK | RRF 排名归一化 [0, 100] |
| 关 | 开 | 向量 → Rerank → topK | Reranker 归一化 [0, 100] |
| 开 | 开 | 向量 + BM25 → RRF → Rerank → topK | Reranker 归一化 [0, 100] |
